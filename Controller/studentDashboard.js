...
const displayProfile = async (req, res) => {
    try {
        ...
        const student = await Student.findAll({ 
            where: { id: userId }, 
            attributes: ['name', 'email', 'phoneNumber', 'role', 'collageId', 'skills'] 
        });
...
const displayReruiter = async (req, res) => {
    ...
    const uploadedData = await Recruiter.findAll({ where: { uploader: collageId } });
  ...
  const editProfile = async(req, res) => {
   try { 
    ...
    if (!editProfile){
      return res.status(400).json({ success: false, message: "Student Id not present"})
    } else {
      let data = await editProfile.update({name:name, email:email, phoneNumber:phoneNumber, skills: skills});
   
    res.status(200).json({success: true, message: 'successfuly updated profile', data})
  } 
}
  catch(err) {
